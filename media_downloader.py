...
DOWNLOAD_DIR = os.path.dirname('/local-media/Entertainment/Comics/Library/ungroupped/')
FAILED_IDS: list = []
DOWNLOADED_IDS: list = []
...
def update_config(config_file: str, config: dict):
    ...
    config["downloaded_ids"] = DOWNLOADED_IDS
    with open(config_file, "w") as yaml_file:
        yaml.dump(config, yaml_file, default_flow_style=False)
    logger.info("Updated last read message_id to config file")

def _has_downloaded(message: pyrogram.types.Message) -> bool:
    """
    Check if the given message id has been downloaded.
    ...
    -------
    bool
        True if the message has downloaded else False.
    """
    return message.id in DOWNLOADED_IDS
...
async def _get_media_meta(
    ...
    if _type in ["voice", "video_note"]:
        ...
        file_name = os.path.join(
            DOWNLOAD_DIR, _type, getattr(media_obj, "file_name", None) or ""
        )
...
async def download_media(
    ...
    for retry in range(3):
        try:
            if message.media is None or _has_downloaded(message):
                return message.id
...
async def begin_import(config_file: str, config: dict, pagination_limit: int) -> dict:
    """
...
def get_project_name(returnAll=False):
    """Function to get list of files to download"""
    configs = glob.glob(os.path.join(THIS_DIR, 'configs/') + '*.yaml')
    configMap = {str(i+1): os.path.basename(configs[i]) for i in range(0, len(configs))}
    
    if returnAll is True:
        return list(configMap.values())
        
    print("\nSelect a config to start with: ")
    for key in configMap:
        print('Enter 0: for all config files')
        print('Enter '+ key+ ': '+configMap[key])
    try:
        choice = input()
        if choice == '0':
            return list(configMap.values())
        else:    
            return [].append(configMap[choice])
    except:
        raise Exception("Invalid Input")
        
def main():
    """Main function of the downloader."""
    for configFileName in get_project_name(len(sys.argv) > 0):
        config_file = os.path.join(THIS_DIR, 'configs', configFileName)
        with open(config_file) as f:
            config = yaml.safe_load(f)
    
        updated_config = asyncio.get_event_loop().run_until_complete(
            begin_import(config_file, config, pagination_limit=100)
        )
        if FAILED_IDS:
            logger.info(
